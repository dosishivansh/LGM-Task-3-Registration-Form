...
const setData = (nam, gend, emai, we, sk, im) => {
    let temp = `<tr class="tabRow"><td><div class="desc"><p id="Sname">${nam}</p><p id="Sgender">${gend}</p><p id="Semail">${emai}</p><p id="Sweb"><a href=${"https://" + we} target="_blank" >${we}</a></p><p id="Sskils">${sk}</p> </div></td><td><div><img src=${im} alt="" onerror="this.onerror=null;this.src='${gend == "Male" ? 'https://cdn.iconscout.com/icon/free/png-256/avatar-370-456322.png' : 'https://st2.depositphotos.com/2703645/9265/v/950/depositphotos_92652082-stock-illustration-woman-avatar-icon.jpg'}';" class="profImg"></div></td></tr>`
